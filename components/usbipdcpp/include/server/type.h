...
namespace usbipdcpp
{
    ...
    {
        std::stringstream ss;
        constexpr std::size_t TRUNCATE_THRESHOLD = 128;
        constexpr std::size_t SHOW_PREFIX = 32;
        auto n = std::ranges::size(t);
        if (n > TRUNCATE_THRESHOLD)
        {
            ss << "[len=" << n << " truncated, prefix=" << SHOW_PREFIX << "] ";
            std::size_t i = 0;
            for (auto it = std::begin(t); it != std::end(t) && i < SHOW_PREFIX; ++it, ++i)
            {
                ss << std::format("{:02x}", static_cast<unsigned>(*it));
                if (i + 1 < SHOW_PREFIX)
                    ss << ' ';
            }
            ss << " ...";
